...
export function createDesigner(vueInstance) {
  ...
  return {
    ...
    checkWidgetMove(evt) { /* Only field widget can be dragged into sub-form */
      if (!!evt.draggedContext && !!evt.draggedContext.element) {
        ...
        if (!!evt.to) {
          ...
          if(evt.draggedContext.element.type !== 'card' && evt.to.className === 'form-widget-list') {
            return false
          }
          if(evt.draggedContext.element.type === 'card' && evt.to.className === 'card-widget-list') {
            return false
          }
        }
      }
    ...
    addContainerByDbClick(container) {
      ...
      if(newCon.type !== 'card')
      this.widgetList[this.widgetList.length - 1].widgetList.push(newCon)
      else this.widgetList.push(newCon)
    ...
    addFieldByDbClick(widget) {
      ...
      } else if(this.widgetList.length) {
        this.widgetList[this.widgetList.length - 1].widgetList.push(newWidget)
      }
